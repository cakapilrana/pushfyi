...
SecureTCPServer* SecureTCPServer::mInstance = 0;

SecureTCPServer* SecureTCPServer::getInstance()
{
    if(0 == mInstance) {
        mInstance = new SecureTCPServer();
    }
    return mInstance;
}

void SecureTCPServer::initializeCommandMap()
{
	mCommandMap[REPORT_STATUS] = eStatus;
    mCommandMap[TCP_READER_ONLINE] = eReaderOnline;
}

SecureTCPServer::SecureTCPServer() : EpollProxy(10000), mThread(0), mTopic(""),
                						mRouter(PubSubRouter::getRouter())
{
    ...
    if(sysProps->contains("pushfyirs.secure.tcp.server.host"))
        setHost(sysProps->get("pushfyirs.secure.tcp.server.host"));
    else
    	/*
     ...
     */
    if(sysProps->contains("pushfyirs.secure.tcp.server.port"))
        setPort(sysProps->getUInt("pushfyirs.secure.tcp.server.port"));
    else
        setPort(443);
     ...
     */
    mTopic          = PUSHFYI_TLS_SERVER_TOPIC;
    ...
    if( pipe(mPipeFd) ) {
		//error creating pipe
		ERROR(Log::eSecureTCPServer, "Error creating pipe. TCPServer can not be sent commands.");
    } else {
        //add to EPOLL SET
		if(addEpollFd(mPipeFd[0]) == 0)
            INFO(Log::eSecureTCPServer, "Notification pipe created. Adding fd = %d to select fd set", mPipeFd[0]);
        else
            ERROR(Log::eSecureTCPServer, "Notification pipe failed to add to EpollProxy. TCPreader can not be sent commands.");
    }
...
SecureTCPServer::~SecureTCPServer()
{
    ...
    if(mThread != 0) {
        ...
        delete mThread;
        mThread = 0;
        INFO(Log::eSecureTCPServer, "TCP Server Thread stoped");
...
bool SecureTCPServer::online()
{
    Synchronized lock(mMutex);
    bool bRet = true;

    if(mThread != 0) {
        INFO(Log::eSecureTCPServer, "Secure TCP Server Thread already running");
        return false;
    }
    ...
    INFO(Log::eSecureTCPServer, "Secure TCP Server Thread started");

    //Good place to start TCP Readers
    Properties* sysProps = ProcessInit::instance()->getSysProps();
    rxUInt count = sysProps->getUInt("pushfyirs.reader.count", 1);

    for(rxUInt i=0; i<count; i++) {
    	//startNewTcpReader();
    }
...
bool SecureTCPServer::offline()
{
    ...
    if(mThread != 0) {
        ...
        delete mThread;
        mThread = 0;
        INFO(Log::eSecureTCPServer, "Secure TCP Server Thread stoped");
    }
 ...
 */
void SecureTCPServer::run()
{
	rxStatus ret = bindAndListen();

	if(ret <0) {
		ERROR(Log::eSecureTCPServer, "Secure TCP Server failed to bind, return = %d", ret);
	} else {
		//add listening socket to EPOLL SET
		INFO(Log::eSecureTCPServer, "Adding listening socket %d to epoll set", mSocket);
		addEpollFd(mSocket);
	}
...
void SecureTCPServer::Accept()
{
    INFO(Log::eSecureTCPServer, "Calling SecureTCPServer::Accept()");
	...
	while(!mFinished) {
         ...
         */
        ret = EpollWait(SecureTCPServer::DEFAULT_EPOLL_TIME_OUT_MILLIS);
        if( ret == -1 ) {
            INFO(Log::eSecureTCPServer, "error occured on epoll_wait, return = %d", ret);
            ...
            for(rxInt i=0; i < ret; ++i) {
                ...
                if( (ev->events & EPOLLERR) || (ev->events & EPOLLHUP) ||
                	...
                	if(ev->data.fd == mSocket) {
                        ERROR(Log::eSecureTCPServer, "We are not listening now. Error occurred on EPOLL FD = %d", ev->data.fd);
                        close();
                ...
                if(ev->data.fd == mPipeFd[0]) {
                	INFO(Log::eSecureTCPServer, "SecureTCPServer Received Push FYI Control Command");
                    char buff[10] = {0};
                ...
                if(ev->data.fd == mSocket) {
                    ...
                    if(newSocket == -1) {
                        ERROR(Log::eSecureTCPServer, "Error in accept, listen address = %s, port = %d epoll_wait returned = %d", mHost.c_str(), mPort, ret);
                        continue;
                    }
                    ...
                    INFO(Log::eSecureTCPServer, "SecureTCPServer received incoming client connection from = %s %d", ip, port);
...
void SecureTCPServer::idle(int retval)
{
//	INFO(Log::eSecureTCPServer, "Secure TCP Server Stats: Idle Cycles = %d, Total Accepts = %d", _IdleCycles, _ConnectionAccepts);
}

void SecureTCPServer::processNewClientSocket(int socketFd, char* ip, rxUShort port)
{
	Synchronized lock(mMutex);
	...
	if(readers > 0)
	{
		...
		newCon->set("security", "tls");

		INFO(Log::eSecureTCPServer, "Sending new client event to TCPReader");
		...
		WARN(Log::eSecureTCPServer, "TCP Readers are currently not available. Can not accept client connection.");
		::close(socketFd);
	}
}

void SecureTCPServer::processCmd()
{
    ...
    for(rxInt i = 0; i < n; i++){
        ...
        if(cmd.empty()){
            WARN(Log::eSecureTCPServer, "Command parameter not set, Topic = %s Subtopic = %s", ev->getTopic().c_str(), ev->getSubtopic().c_str());
            continue;
        }

        INFO(Log::eSecureTCPServer, "Processing command: %s", cmd.c_str());
        switch(mCommandMap[cmd]) {
			...
			default:
			{
                WARN(Log::eSecureTCPServer, "Unknown Command = %s, Topic = %s Subtopic = %s", cmd.c_str(), ev->getTopic().c_str(), ev->getSubtopic().c_str());
			}
			break;
        } //end of switch
    }
...
void SecureTCPServer::notify()
{
    //if the write pipe fd is open, write 1 char for select to wake up
    if(mPipeFd[1] > 0) {
        const char* c = "x";
        ::write(mPipeFd[1], c, 1);
        INFO(Log::eSecureTCPServer, "SecureTCPServer command notified.");
    } else {
        ERROR(Log::eSecureTCPServer, "SecureTCPServer notification pipes not open. Command notification failed.");
    }
}

void SecureTCPServer::push(Event event)
{
    submitCmd(event);
}

void SecureTCPServer::pushFront(Event event)
{
    submitCmd(event, true);
}

void SecureTCPServer::submitCmd(Event event, bool isHighPriority)
{
    if (isHighPriority) {
...
void SecureTCPServer::subscribe()
{
    mRouter.subscribe("", "", this, mTopic, "", -1);
}

void SecureTCPServer::unsubscribe()
{
    mRouter.unsubscribe("", "", this, mTopic);
}

string SecureTCPServer::getUUID()
{
    char   uuidStr[64] = {0};
    uuid_t uuid;
...
void SecureTCPServer::processReaderOnline(Event& ev)
{
	Synchronized lock(mMutex);
